...
public class LogIn {
    ...
    public Administrador inicioSesionAdmin(String nombreArchivo) throws FileNotFoundException {
        ...
        ArrayList<Administrador> administradores = GestionJSONAdministrador.archivoAdminToLista(nombreArchivo); //hay que volver a serializarlo?
        ...
        while (adminLeido == null) {
            ...
            for (int i = 0; i < administradores.size(); i++) {
                if (administradores.get(i).getUserName().equals(username) && administradores.get(i).getContrasena().equals(contrasenia) && administradores.get(i).isActivo()) {
                    adminLeido = new Administrador();
                    return adminLeido;

                }else if(administradores.get(i).getUserName().equals(username) && administradores.get(i).getContrasena().equals(contrasenia) && !administradores.get(i).isActivo()){
                    System.out.println("Su cuenta se encuentra dada de baja");

                }else if(administradores.get(i).getUserName().equals(username) || administradores.get(i).getContrasena().equals(contrasenia)){
